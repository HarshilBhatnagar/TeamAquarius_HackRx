...
class TextAgent:
    ...
    async def get_answer(self, question: str, document_content: Any) -> str:
        ...
        try:
            ...
            # SIMPLIFIED RETRIEVAL: Focus on direct question matching
            logger.info(f"Text Agent: Starting simplified retrieval for question: '{question}'")
            
            # Use direct question retrieval with higher k value
            initial_chunks = await asyncio.to_thread(self.ensemble_retriever.invoke, question)
            logger.info(f"Text Agent: Direct retrieval returned {len(initial_chunks)} chunks")
            
            # If we don't get enough chunks, try with expanded keywords
            if len(initial_chunks) < 10:
                question_lower = question.lower()
                expanded_queries = []
                
                # Add specific keywords based on question type
                if 'waiting period' in question_lower or 'pre-existing' in question_lower:
                    expanded_queries = ['36 months', 'pre-existing disease', 'waiting period', 'continuous coverage']
                elif 'child' in question_lower or 'hospitalization' in question_lower:
                    expanded_queries = ['child', 'hospitalization', 'cash benefit', 'accompanying']
                elif 'surgery' in question_lower or 'hernia' in question_lower:
                    expanded_queries = ['surgery', 'hernia', 'surgical', 'coverage']
                elif 'organ donor' in question_lower:
                    expanded_queries = ['organ donor', 'pre-hospitalization', 'post-hospitalization', 'donor']
                
                # Get additional chunks from expanded queries
                for query in expanded_queries:
                    try:
                        additional_chunks = await asyncio.to_thread(self.ensemble_retriever.invoke, query)
                        initial_chunks.extend(additional_chunks)
                    except Exception as e:
                ...
                initial_chunks = unique_chunks
            
            logger.info(f"Text Agent: Final retrieval returned {len(initial_chunks)} unique chunks")
